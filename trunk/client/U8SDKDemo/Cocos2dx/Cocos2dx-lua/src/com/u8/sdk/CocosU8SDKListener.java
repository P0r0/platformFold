...
public class CocosU8SDKListener implements IU8SDKListener{
			...
			public void run() {
				...
				case U8Code.CODE_INIT_SUCCESS:
					...
					try {
						...
						json.put("operatorName", U8SDK.getInstance().getOperators());
					} catch (JSONException e) {
						
						e.printStackTrace();
					}
					
					context.sendInitCallback(U8CocosActivity.CALLBACK_INIT, json);
					break;
	...
	public void onPayResult(PayResult result) {
		...
		try {
			...
			json.put("productID", result.getProductID());
			json.put("productName", result.getProductName());
			json.put("extension", result.getExtension());

			context.sendCallback(U8CocosActivity.CALLBACK_PAY,json);
		} catch (Exception e) {
		}
	}
