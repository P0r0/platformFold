...
public class SDKInterfaceAndroid : U8SDKInterface
{
    ...
    private void SDKCall(string method, params object[] param)
    {
        try
        {
            Debug.Log("Calling Android Method: "+method);
    ...
    public override bool IsSuportLogin()
    {
        return SDKCall<bool>("isSupportLogin");
    }
    ...
    private string encodePayParams(U8PayParams data)
    {
        ...
        map.Add("egameCode", data.egameCode);
        map.Add("unicomCode", data.unicomCode);
        map.Add("mmCode", data.mmCode);
        map.Add("meegoCode", data.meegoCode);
