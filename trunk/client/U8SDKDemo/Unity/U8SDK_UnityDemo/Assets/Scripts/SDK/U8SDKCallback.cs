...
public class U8SDKCallback : MonoBehaviour
{
    ...
    {
        //Nothing...
        U8PayResult data = parsePayResult(jsonData);
        if (data == null)
        {
            UnityEngine.Debug.LogError("The data parse error." + jsonData);
            return;
        ...
        if (U8SDKInterface.Instance.OnPaySuc != null)
        {
            U8SDKInterface.Instance.OnPaySuc.Invoke(data);
        }
    }

    private U8PayResult parsePayResult(string str)
    {
        object jsonParsed = MiniJSON.Json.Deserialize(str);
        if (jsonParsed != null)
        {
            Dictionary<string, object> jsonmap = jsonParsed as Dictionary<string, object>;
            U8PayResult data = new U8PayResult();
            ...
            {
                data.productID = jsonmap["productName"].ToString();
            }
            if (jsonmap.ContainsKey("orderID"))
                data.orderID = jsonmap["orderID"].ToString();

            if (jsonmap.ContainsKey("extension"))
                data.orderID = jsonmap["extension"].ToString();
            return data;
        }
        return null;
    }
