 ...
 */
    ...
    void onPaySuccessful(string jsonString){
		...
		Dictionary<string,object> result = MiniJSON.Json.Deserialize (jsonString) as Dictionary<string,object>;
		if(result.ContainsKey(Response.TYPE)){
			...
			if(type == Response.LISTENER_TYPE_PAY){
				...
				if(result.ContainsKey(Response.DATA)){
					...
					Dictionary<string,object> billDetail = MiniJSON.Json.Deserialize (data) as Dictionary<string,object>;
